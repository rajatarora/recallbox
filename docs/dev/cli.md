...
The CLI application is defined in `recallbox/cli.py` and automatically configured as an entry point in `pyproject.toml`:

```toml
[project.scripts]
recallbox = "recallbox.cli:app"
```

After installation, the CLI is available as the `recallbox` command.
...
```bash
recallbox my-command --option value argument
```
...
Add a basic command to `recallbox/cli.py`:
...
```bash
recallbox process input.txt --output output.txt --verbose
recallbox process input.txt -o output.txt -v
```
...
```bash
recallbox export --count 50 --format yaml --output data.yaml
```
...
```python
from recallbox.services.db import get_engine
...
```bash
recallbox db migrate
recallbox db seed
recallbox user create john john@example.com
recallbox user list
```
...
```python
from recallbox.commands import database, users
...
from recallbox.cli import app, syncify
import asyncio
...
def test_version_command_runs():
    ...
    assert "recallbox" in result.stdout.lower()
