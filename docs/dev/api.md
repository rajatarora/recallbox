...
The FastAPI application is defined in `recallbox/www.py` and includes:

- **Automatic API documentation** at `/docs` (Swagger UI) and `/redoc` (ReDoc)
- **Static file serving** from `recallbox/static/` via the `/static/` endpoint
...
Create a new route in `recallbox/www.py`:
...
from recallbox.services.db import get_session_depends
...
Static files are served from `recallbox/static/`:
...
uvicorn recallbox.www:app --reload --host 0.0.0.0 --port 8000
...
uvicorn recallbox.www:app --host 0.0.0.0 --port 8000 --workers 4

# Or using gunicorn with uvicorn workers
gunicorn recallbox.www:app -w 4 -k uvicorn.workers.UvicornWorker
```
