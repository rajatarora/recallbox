...
ENV APP_MODULE=recallbox.www:app
...
```yaml
volumes:
  - "./recallbox:/app/recallbox"  # Source code
...
docker build -f dockerfile.www -t recallbox-www:latest .

# Tag for registry
docker tag recallbox-www:latest ghcr.io/your-org/recallbox-www:latest

# Push to registry
docker push ghcr.io/your-org/recallbox-www:latest
```
...
FROM ghcr.io/multi-py/python-uvicorn:py3.14-slim-LATEST AS builder
...
FROM ghcr.io/multi-py/python-uvicorn:py3.14-slim-LATEST
...
if [ ! -z "$CREATE_TEST_DATA" ]; then
  ...
  python -m recallbox.cli test-data
fi
...
services:
  www:
    image: ghcr.io/your-org/recallbox-www:latest
    restart: always
...
# Run a command
docker-compose exec www python -m recallbox.cli version

# Check database connection
docker-compose exec www python -c "from recallbox.services.db import engine; print(engine)"
```
