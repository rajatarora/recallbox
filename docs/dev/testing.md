...
pytest --cov=./recallbox --cov-report=term-missing tests

# Generate HTML coverage report
pytest --cov=./recallbox --cov-report=html tests
# Open htmlcov/index.html in your browser

# Show coverage for specific file
pytest --cov=./recallbox/services/cache.py tests/services/test_cache.py
```
...
def test_version_command():
    ...
    assert "recallbox" in result.stdout
```
...
```python
@pytest.mark.asyncio
@patch('recallbox.services.cache.get_cached')
...
async def test_with_custom_env():
    ...
    from recallbox.settings import settings
...
def test_settings_load():
    ...
    assert settings.project_name == "recallbox"
...
pytest --cov=./recallbox --cov-fail-under=80 tests
```
